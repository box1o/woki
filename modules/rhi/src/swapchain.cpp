...
Frame Swapchain::MakeFrame(
    scope<TextureView> color_view, TextureView* depth_view, const u32 width, const u32 height) {
    Frame frame{};
...
TextureView* Frame::DepthView() noexcept { return depth_view_; }

const TextureView* Frame::DepthView() const noexcept { return depth_view_; }
...
Result<scope<Swapchain>> Swapchain::Builder::Build() {
    if (device_ == nullptr || surface_ == nullptr) {
        return Err(ErrorCode::GraphicsResourceCreationFailed,
            "Swapchain builder requires device and surface");
    }
