...
Result<RenderFramePlan> BuildRenderFramePlan(
    ...
    auto opaque = BuildRenderQueue(snapshot, {.phase = DrawPhase::Opaque, .frustum = view.frustum});
    if (!opaque) {
        return Err(opaque.error());
    }
    auto transparent =
        BuildRenderQueue(snapshot, {.phase = DrawPhase::Transparent, .frustum = view.frustum});
    if (!transparent) {
