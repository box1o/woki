...
TEST_CASE("Standard shader library describes full textured PBR frame bindings") {
    const woki::gfx::StandardShaderLibrary library{"shaders"};
    const auto shader = library.Describe(woki::gfx::StandardShader::PbrFull);

    REQUIRE(shader.asset_id == woki::gfx::AssetId{"woki/shaders/pbr_full"});
    REQUIRE(shader.sources.front().source_path == "shaders/pbr_full_forward.wgsl");
    REQUIRE(shader.interface.uses_lighting);
    REQUIRE(shader.interface.uses_shadows);
    REQUIRE(shader.interface.uses_environment);
    REQUIRE(shader.interface.parameters.size() == 7);
