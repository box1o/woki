...
template <typename T, typename Tag> class Pool final {
public:
    Pool() = default;
    ...
    [[nodiscard]] Handle<Tag> Emplace(Args&&... args) {
        if (!free_indices_.empty()) {
            ...
            ++size_;
            return Handle<Tag>::FromParts(index, slot.generation);
        }
        ...
        ++size_;
        return Handle<Tag>::FromParts(index, slots_.back().generation);
    }

    [[nodiscard]] bool Contains(const Handle<Tag> handle) const noexcept {
    ...
    [[nodiscard]] T* TryGet(const Handle<Tag> handle) noexcept {
        if (!Contains(handle)) {
            return nullptr;
        }
    ...
    [[nodiscard]] const T* TryGet(const Handle<Tag> handle) const noexcept {
        if (!Contains(handle)) {
            return nullptr;
        }
    ...
    [[nodiscard]] T& Get(const Handle<Tag> handle) {
        T* value = TryGet(handle);
        WOKI_ASSERT_MSG(value != nullptr, "Invalid resource handle");
        return *value;
    }

    [[nodiscard]] const T& Get(const Handle<Tag> handle) const {
    ...
    template <typename Function> void Each(Function&& function) {
        for (u32 index = 0; index < slots_.size(); ++index) {
            ...
            if (slot.value.has_value()) {
                std::invoke(function, Handle<Tag>::FromParts(index, slot.generation), *slot.value);
            }
        }
    }
...
private:
    ...
    [[nodiscard]] static constexpr u32 NextGeneration(const u32 generation) noexcept {
        ...
        return next == Handle<Tag>::kInvalidGeneration ? 1u : next;
    }
